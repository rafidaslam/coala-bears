...
def submit_youtube_to_archive_org(url):
    """
    ...
    try:
        with patch.object(sys, 'argv', ['tubeup', url]):
            # Redirect stdout and stderr that coming from
            # `tubeup.__main__.main()`
        ...
        logging.error(er)
        logging.error('Error has been occured from tubeup\n'
                      'Here is the log:\n%s' % f.getvalue())
        return False

    if re.search(':: Upload Finished\. Item information:', f.getvalue()):
        return True
    logging.error('tubeup doesn\'t tell the program whether the uploading '
                  'process has been done or not. \n'
